	...
	class CBSB_Expedited_Booking {
		...
		function select_single_service( $wp_data ) {
			...
			if ( isset( $services['status'] ) && 'success' == $services['status'] ) {
				if ( isset( $wp_data['initialState'] ) && isset( $services['data'] ) && ! is_null( $services['data'] ) &&	property_exists( $services['data'], 'all_services' ) ) {
		...
		function single_service_type( $wp_data ) {
			$services = $this->services();
			if ( isset( $services['status'] ) && 'success' == $services['status'] ) {
				if ( isset( $wp_data['initialState'] ) && isset( $services['data'] ) && ! is_null( $services['data'] ) && property_exists( $services['data'], 'all_services' ) && count( $services['data']->service_types ) == 1 ) {
		...
		function qty_services( $wp_data ) {
			...
			if ( isset( $services['status'] ) && 'success' == $services['status'] ) {
				if ( isset( $wp_data['initialState'] ) && isset( $services['data'] ) && ! is_null( $services['data'] ) && property_exists( $services['data'], 'all_services' ) && count( $services['data']->all_services ) < 5 ) {
					$wp_data['initialState']->service_types = array( 'all' => $services['data']->all_services );
					$wp_data['initialState']->available_services = (array) $services['data']->all_services;
					$wp_data['initialState']->all_services = (array) $services['data']->all_services;
